...
// React native fs module expose only async functions,
// so we can't use nodejs sync versions.
let statSync        = fs.statSync || deasync(fs.stat),
    readFileSync    = fs.readFileSync || deasync(fs.readFile);
    ...
    loadConfigFiles() {
        ...
        lodash.forEach(FILENAMES, (filename) => {
            lodash.forEach(extensions, (extension) => {
                ...
                try {
                    fileContent = readFileSync(fullFilename, 'UTF-8');
